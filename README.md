...
> **Project status:** `osqar.inspector.config.v1` resolution, `osqar.inspector.snapshot.v1` clean-Git capture/materialization, `osqar.inspector.plan.v1`, the library-level `builtin.doxygen.v1` producer adapter, generic pre-generated coverage ingestion with `osqar.inspector.coverage-map.v1` and `osqar.inspector.coverage-attestation.v1`, and `osqar-inspector verify --bundle PATH` are implemented. The Doxygen adapter records producer execution and mechanically validated mappings; it does not review or approve generated documentation. CLI-level `build`, publication, coverage-producer execution, signatures, and OSQAr integration remain design targets.
